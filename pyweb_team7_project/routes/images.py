...
from ..schemas import CreateImageModel, UpdateImageModel
router = APIRouter(prefix='/images', tags=['images'])


@router.post("/", response_model=Image, status_code=status.HTTP_201_CREATED, dependencies=[Depends(RateLimiter(times=2, seconds=5))])
...
@router.get("/{image_id}", response_model=Image, dependencies=[Depends(RateLimiter(times=2, seconds=5))])
